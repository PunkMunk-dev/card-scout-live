    ...
    <div className="mt-1">
      <div className="flex items-center gap-1">
        <button
          ...
          title="Map PSA population"
          className="p-0.5 rounded hover:opacity-70"
        >
          <MapPin className="h-2.5 w-2.5" style={{ color: 'var(--om-accent)' }} />
        </button>
        <button
          onClick={() => invokeMapping('verify_psa_mapping')}
          title="Verify PSA mapping"
          className="p-0.5 rounded hover:opacity-70"
        >
      ...
      {showForm && (
        <div className="mt-1 space-y-1">
          <input
            className="w-full text-[9px] px-1 py-0.5 rounded"
            style={{ background: 'var(--om-bg-1)', border: '1px solid var(--om-border-0)', color: 'var(--om-text-0)' }}
            placeholder="PSA Set Name"
            value={psaSet}
            onChange={(e) => setPsaSet(e.target.value)}
          />
          <input
            className="w-full text-[9px] px-1 py-0.5 rounded"
            style={{ background: 'var(--om-bg-1)', border: '1px solid var(--om-border-0)', color: 'var(--om-text-0)' }}
            placeholder="PSA Subject"
            value={psaSubject}
            onChange={(e) => setPsaSubject(e.target.value)}
          />
          <input
            className="w-full text-[9px] px-1 py-0.5 rounded"
            style={{ background: 'var(--om-bg-1)', border: '1px solid var(--om-border-0)', color: 'var(--om-text-0)' }}
            ...
            value={psaCardNum}
            onChange={(e) => setPsaCardNum(e.target.value)}
          />
          <button
